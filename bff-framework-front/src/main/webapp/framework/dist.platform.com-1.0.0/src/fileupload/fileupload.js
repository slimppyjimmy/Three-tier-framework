...
angular.module('dist.ui.fileupload', ['dist.ui.position', 'dist.ui.progress'])
     ...
     * @param {string} wiTrigger 触发文件选择弹窗的元素 id，该元素仅用于获得位置和大小，从而让实际的input覆盖在此元素上
    ...
    .directive('wiFileupload', ['$position', 'fileuploadSev', 'fileuploadConf', '$timeout', '$parse', '$document', 'wiDialog', function ($position, fileuploadSev, fileuploadConf, $timeout, $parse, $document, wiDialog) {
        return {
            restrict: 'E',
            templateUrl: 'framework/dist.platform.com-1.0.0/src/fileupload/template/fileupload/wi-fileupload.html',// 模态时调用 dialog 处需将模板以字符串形式传入
            scope: {},
            replace: true,
            link: function (scope, elem, attrs) {
                ...
                var options = scope.fileULOptions = {
                    url: attrs['wiUrl'] || ''
                    , multi_selection: attrs.hasOwnProperty('wiMultisel') && attrs['wiMultisel'] == 'true'// 是否允许多选，默认不支持
                    , browse_button: document.getElementById(attrs['wiTrigger'])// 触发选择文件弹窗的元素
                    , modal: attrs.hasOwnProperty('wiModal') && attrs['wiModal'] == 'true'// 是否模态显示上传状态，默认非模态
                };
...
//                options.url = "http://192.10.110.174:8804/NewFrame/CommonServlet?bean=FileManagerPO&path=D:\\wisoft\\&sid=408aee4e3bacd24f013bb0ec0e650030&isUploadMore=true&uploadMaxSize=8000000"; // 服务器测试
                ...
                // 每个上传文件的最大值
                if (attrs['wiMaxSize']) {
                    options.max_file_size = fileuploadSev.parseSize(attrs['wiMaxSize']);
                    options.max_file_size_str = fileuploadSev.formatSize(options.max_file_size);
                }
                // ie9 下打开的 swf 路径
                ...
                var triggerPos = $position.position(angular.element(options.browse_button))
                    ...
                    ,
                    triggerInput = angular.element('<input type="file" class="wi-fileupload-btn"' + elemStr.multiple + elemStr.accept + ' />')
                    ,
                    triggerDiv = angular.element('<div class="wi-fileupload-btndiv" style="' + elemStr.pos + '"></div>');
                elem.context.style.display = "none";
                // 获取选中的文件
                triggerInput.on('change', function g() {
                    if (!xhr) return;
                    scope.files = [];
                    uploadFiles = [];
                    ...
                    try {// 清空控件中的值 - 确保下次选择文件后肯定触发change事件
                        this.files = [];
                        this.value = '';
                    } catch (e) {// 兼容处理，部分老的浏览器不支持对 this.files 及 this.value 赋值，删除并重新创建input，如果不删除，modal==true时会出现多个modal，如果不新建，当所选文件与上次相同时将不会触发change事件
                        triggerInput.remove();
                        triggerInput = null;
                        triggerInput = angular.element('<input type="file" class="wi-fileupload-btn"' + elemStr.multiple + elemStr.accept + ' />');
                        triggerInput.on('change', g);
                        triggerDiv.append(triggerInput);
                    }
                    scope.$digest();
                    if (scope.files.length > 0) {
                        ...
                        if (options.modal) {
                            ...
                            modalDialogId = wiDialog.open({
                                ...
                                //                 <span class="wi-fileupload-error" ng-style="{display: file.stat==5 ? \'\' : \'none\'}">文件大小超过&nbsp;{{::fileULOptions.max_file_size_str}}&nbsp;限制</span>\n                                        <span ng-if="file.stat==1 || file.stat==2" class="wi-fileupload-del" ng-click="removeFile(file)">取消</span>\
                                //             </li>\
                                //         </ul>',
                                //     plain: true,
                                template: 'framework/dist.platform.com-1.0.0/src/fileupload/template/fileupload/wi-fileupload.html',
                                plain: false,
                                title: '上传进度',
                                scope: scope,
                                width: 380
                            ...
                            elem.context.style.display = "block";
                        }
                    }
                    queueCtrl();// 上传队列处理
                });
                ...
                scope.$on('$destroy', function () {
                    ...
                    triggerInput.remove();// 移除
                    triggerDiv.remove();
                    triggerInput = null;// 销毁
                    triggerDiv = null;
                });
                /* --------- 上传配置 --------- */
                var xhr,
...
//                    });
                ...
                function addFile(file) {
                    ...
                    var f = {
                        ...
                        percent: 0,
                        progressLabel: '0%',
                        stat: options.max_file_size && file.size > options.max_file_size ? fileuploadConf.exceeded : fileuploadConf.queued // 等待上传
                    };
                    file.wiId = f.id;
                    scope.files.push(f);
