describe('carousel', function() {
  beforeEach(module('dist.ui.carousel', function($compileProvider, $provide) {
  ...
  describe('basics', function() {
    ...
    beforeEach(function() {
      ...
      elm = $compile(
        '<wi-carousel interval="interval" no-transition="true" no-pause="nopause">' +
    ...
    it('should display clickable slide indicators', function () {
      var indicators = elm.find('ol.indicators > li');
    ...
    it('should hide navigation when only one slide', function () {
      ...
      scope.$apply();
      elm = $compile(
          '<wi-carousel interval="interval" no-transition="true">' +
            '<wi-slide ng-repeat="slide in slides" active="slide.active">' +
              '{{slide.content}}' +
            '</wi-slide>' +
          '</wi-carousel>'
    ...
    it('should bind the content to slides', function() {
      var contents = elm.find('div.wi-slide');
    ...
    it('should remove slide from dom and change active slide', function() {
      ...
      expect(elm.find('div.wi-slide').length).toBe(2);
      testSlideActive(1);
      $timeout.flush();
      testSlideActive(0);
      scope.$apply('slides.splice(1,1)');
      expect(elm.find('div.wi-slide').length).toBe(1);
      testSlideActive(0);
    });
