...
angular.module('dist.ui.tree',[])
     ...
     * @param {function=} onitemmouseenter 鼠标移入节点时的回调函数:onitemmouseenter(node).
     * @param {function=} onitemmouseleave 鼠标移出节点时的回调函数:onitemmouseleave(node).
     */
    .directive('wiTree',['RecursionHelper','treeService','$filter','$parse',function (RecursionHelper,treeService,$filter,$parse) {
        return {
            ...
            templateUrl: function(ele,attr){
                return 'framework/dist.platform.com-1.0.0/src/tree/template/tree/treeTemplate.html';
            },
            replace:true,
            scope: {
                // Properties
...
//                ,orderby:'@'// 排序字段，默认'id' TODO
            ...
            controller: ['$scope','treeService', function ($scope,treeService) {
                ...
                ctrl.handler = {};
                ctrl._hashData = {};
                ctrl.precondition=$scope.precondition;
                ctrl.settingicoprecondition=$scope.settingicoprecondition;
                /**
...
//                        node['__current']=false;
                        ...
                        if ($scope.listtype && $scope.listtype == 3){ // 异步加载：需每次求情数据
                            if (node['children']) {
                                node['isbranch']=true;
                                node['children'] = [];
                                node['__closed'] = true;
                            }
                        }else if ($scope.listtype && $scope.listtype == 2) { // 异步加载：需第一次请求所有数据
                            if (node['children']) {
            ...
            compile: function(element) {
                return {
                    post: function(scope,element,attrs,ctrl,linker){
...
//                            conf.orderby = scope.orderby || conf.idfield;
                            ...
                            conf.onitemmouseenter = scope.onitemmouseenter() || angular.noop;
                            conf.onitemmouseleave = scope.onitemmouseleave() || angular.noop;
                            /* 多选支持 */
                            (function(){
                                var _setMultisel;
                            ...
                            var expandNode = function (node) {
                                ...
                                node['__closed'] = false;
                                if (node['isbranch'] && node['children'] && !node['children'].length) { // 每次展开要重新请求children数据
                                    node['__loading'] = true;
                                    conf.onloadbranch(node, function (children) { // 异步加载成功的回调函数
                                        node['__loading'] = false;
                                        if (angular.isArray(children)) {
                                            registerNode(children, node);
                                        }
                                        conf.onexpand(node);
                                    }, function (err) { // 异步加载失败的回调函数
                                        console.log('onloadbranch异步加载失败', err);
                                    });
                                } else { //默认情况：listtype==1 或者 listtype==2时
                                    conf.onexpand(node);
                                }
                            };
                            // 折叠 node
                            ...
                            handler.clickRow = function(node){
                                ...
                                if (scope.disabled=='disabled') return;
                                ctrl.changeCur(node);// 切换高亮
                                // var oldStatus = node['__selected'];
                            ...
                            //checkbox里面绑定点击事件
                            handler.clickInput=function(node){
                                conf.onitemclick(node);
                                if (scope.disabled=='disabled') return;
                                var oldStatus = node['__selected'];
                                if(oldStatus){// 已被选中，单选不作处理，多选时反转选中状态
                        ...
                        // 根据条件给tree控件标记不同的样式
                        vm._precondition=ctrl.precondition || false;
                        vm._settingicoprecondition=ctrl.settingicoprecondition || false;
                        vm._getConditionResult=function (precondition, row) {
                            if (!precondition.conditions) return;
                            var isTrue,
                                logicType=precondition.logicType || 'or';
                            if (logicType=="or"){
                                ...
                                console.log('vm.getConditionResult,配置了错误的logicType，只能是：and/or');
                            }
                        };
                    }
                };
